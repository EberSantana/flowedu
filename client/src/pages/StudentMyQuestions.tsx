              ...
              <div className="flex flex-wrap gap-4">
                ...
                {statusFilter && (
                  <div className="flex items-end">
                    <Button variant="outline" onClick={() => setStatusFilter(undefined)}>Limpar Filtro</Button>
                  </div>
                )}
              </div>
            </CardContent>
          ...
          <div className="space-y-4">
              ...
              <Card>
                <CardContent className="py-12 text-center">
                  ...
                  <Link href="/student/submit-question">
                    <Button><Plus className="h-4 w-4 mr-2" />Enviar Primeira Dúvida</Button>
                  </Link>
                </CardContent>
              </Card>
            )}
            {questions?.map((item) => (
              <Card key={item.question.id} className="hover:shadow-md transition-shadow">
                      ...
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        {item.question.viewCount > 0 && (
                          <Badge variant="outline" className="text-xs flex items-center gap-1">
                            <Eye className="h-3 w-3" />{item.question.viewCount}
                          </Badge>
                        )}
                      </div>
                      <CardTitle className="text-xl mb-2">{item.question.title}</CardTitle>
                      <CardDescription className="flex flex-wrap items-center gap-4 text-sm">
                        <span className="flex items-center gap-1"><BookOpen className="h-3 w-3" />{item.subject?.name || 'Disciplina'}</span>
                ...
                <CardContent>
                  <p className="text-sm text-muted-foreground line-clamp-2">{item.question.content}</p>
                </CardContent>
              </Card>
            ))}
          </div>
